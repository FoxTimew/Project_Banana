...
public class WallReboundE : MonoBehaviour
{
    ...
    void DirectionAtribution(Collider2D col, int player)
    {
        if (player == 1)
        {
            ...
            {
                col.gameObject.GetComponent<Controler>().pousseeDirection = Vector2.left;
            }
        }
        else if (player == 2)
        {
        ...
        else
        {
            ...
            {
                col.gameObject.GetComponentInParent<Barril_Sys>().direction = Vector2.left;
            }
        }
    }
}
